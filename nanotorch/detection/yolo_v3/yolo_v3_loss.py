...
3. Classification loss (multi-label using binary cross-entropy)

Key differences from YOLO v1:
- Uses sigmoid instead of softmax for classification
- Predicts offsets from anchors
...
class YOLOv3Loss(Module):
    ...
    - Objectness loss (BCE on confidence)
                ...
                for j in range(W):
                    for a in range(num_anchors):
                        ...
                        for t_idx, box in enumerate(boxes):
                            ...
                            iou = self._compute_iou_center(
                                cell_x, cell_y, 
                                anchor_w / 416, anchor_h / 416,
                                gt_cx, gt_cy, gt_w / 416, gt_h / 416
                            )
                        ...
                        if best_iou > self.ignore_threshold and best_target_idx >= 0:
                            ...
                            coord_loss += (pred_tx - target_tx) ** 2
                            coord_loss += (pred_ty - target_ty) ** 2
                            coord_loss += (pred_tw - target_tw) ** 2
                            coord_loss += (pred_th - target_th) ** 2
                            coord_count += 4
    ...
    def _compute_iou_center(
        self,
        cx1: float, cy1: float, w1: float, h1: float,
        cx2: float, cy2: float, w2: float, h2: float
    ) -> float:
        """Compute IoU between two boxes given center coordinates."""
        x1_min = cx1 - w1 / 2
        y1_min = cy1 - h1 / 2
        x1_max = cx1 + w1 / 2
        y1_max = cy1 + h1 / 2
...
class YOLOv3LossSimple(Module):
    """Simplified YOLO v3 Loss for testing."""
...
def encode_targets_v3(
    ...
    """Encode ground truth to YOLO v3 format.
...
def decode_predictions_v3(
    ...
    """Decode YOLO v3 predictions to bounding boxes.
