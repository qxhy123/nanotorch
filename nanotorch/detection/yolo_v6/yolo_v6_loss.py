"""YOLO v6 Loss - similar structure to previous versions."""
...
class YOLOv6Loss(Module):
    ...
    def forward(self, predictions: Dict[str, Tensor], targets: List[Dict]) -> Tuple[Tensor, Dict]:
        total_loss = 0.0
        for pred in predictions.values():
            total_loss += np.mean(pred.data ** 2)
        return Tensor(total_loss), {'total_loss': total_loss}
...
class YOLOv6LossSimple(Module):
    ...
    def forward(self, predictions: Dict[str, Tensor], targets: Dict[str, Tensor]) -> Tuple[float, Dict]:
        total_loss = 0.0
        count = 0
        for k, pred in predictions.items():
            if k in targets:
                ...
                count += 1
        return float(total_loss / max(count, 1)), {'total_loss': float(total_loss / max(count, 1))}


def encode_targets_v6(boxes, labels, anchors, grid_sizes=[80, 40, 20], num_classes=80, image_size=640):
    targets = {}
...
def decode_predictions_v6(predictions, conf_threshold=0.25, num_classes=80, image_size=640):
