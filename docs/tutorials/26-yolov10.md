...
模型输出满满当当的检测结果——一百个、两百个框。但真正有用的，可能只有十来个。

剩下的？都是重复的、冗余的，需要用一种叫 **NMS** 的后处理方法来过滤。

这个过程就像考试交卷前的检查——明明已经写完答案了，却还要花时间去涂改、去筛选。更烦人的是，这个"尾巴"拖慢了整个流程。

**能不能，一次就交出完美答卷？**

YOLO v10 做到了。它用**一致双重分配**策略，在训练时就学会了"一个目标只预测一次"。推理时，直接输出最终结果，不再需要任何后处理。

```
传统 YOLO：
  检测 100 个框 → NMS 过滤 → 保留 10 个
  多了最后一步，就多了延迟
