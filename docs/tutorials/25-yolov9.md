...
深度网络也是如此。每一层都会丢失一些信息，梯度从深层反向传播到浅层时，也常常面目全非。网络越深，这个问题越严重。

YOLO v9 带来了一个天才的解决方案：**可编程梯度信息（PGI）**。

它为网络开辟了一条"秘密通道"——让完整的原始信息，直接传送到网络的每一层。就像在传话游戏中，让每个人都能直接听到第一个人的声音。
