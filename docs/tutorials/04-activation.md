...
山峰？是一段斜坡。波浪？是几个折线。人脸？是无数个小三角形的拼接。

这不是不可能，但太笨拙了。你需要无数条直线，才能勉强逼近一条曲线。

神经网络的尴尬处境也是如此。如果没有激活函数，无论叠加多少层，它本质上还是在画直线。
...
激活函数，就是那条**让直线弯曲的魔法**。它在每一层之间，插入了非线性，让网络拥有了拟合任意形状的能力。

```
ReLU：把负数斩断，只留下正数
Sigmoid：把一切压缩到 0 和 1 之间
Tanh：在 -1 和 1 之间起伏

直线 → 折线 → 曲线 → 万物
```

**激活函数，是神经网络的灵魂。** 没有它，网络只是一堆线性变换，再深也学不会复杂的世界。
