...
一张图片是什么？是 224×224×3 个数字的阵列。

一段文字是什么？是一串词向量的序列。

一个声音是什么？是振幅随时间变化的波形。

神经网络看不见图像，听不见声音，也读不懂文字。它唯一能理解的，是**数字**。而 Tensor，就是承载这些数字的容器。

想象一个魔方——长、宽、高，三个维度。现在，把这个魔方无限延展：四个维度、五个维度...甚至一百个维度。这就是 Tensor。
...
2 维：一张棋盘  → 矩阵 ([[1,2],[3,4]])
3 维：一本相册  → 张量 ([[[...]]])
4 维：一段视频  → 批量×时间×高×宽×通道
```

**Tensor，是数字在计算机中的万种形态。**
