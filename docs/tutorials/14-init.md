...
在训练开始之前，神经网络的参数都是随机的。

但"随机"和"随机"不一样。

如果全设为零，所有神经元都在做同样的事，学着同样的东西，最后也变得一模一样。这叫"对称性问题"——就像一百个学生抄同一份作业，谁也没学到东西。

如果随机得太离谱，数值可能太大或太小。太大会导致梯度爆炸，太小会导致梯度消失。训练还没开始，就已经失败了。

**好的初始化，是在"多样性"和"数值稳定"之间找到平衡。**
