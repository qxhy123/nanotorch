...
想象你在经营一家奶茶店，利润受到多种因素影响：
- **价格**：涨价会怎样？
- **温度**：天气热会怎样？
- **位置**：开在商场还是学校？

**向量导数就是**：同时追踪**所有因素**对结果的影响！

```
利润 = f(价格, 温度, 广告费, ...)

梯度 = [∂利润/∂价格, ∂利润/∂温度, ∂利润/∂广告费, ...]
       ↑          ↑          ↑
     价格影响   温度影响    广告影响
```

**核心思想**：梯度告诉你每个因素对结果的影响程度。

### 📝 手把手计算向量导数

**例子**：$f(x_1, x_2) = x_1^2 + 2x_2$
...
**定理**：设 $\mathbf{g}: \mathbb{R}^n \to \mathbb{R}^m$，$\mathbf{f}: \mathbb{R}^m \to \mathbb{R}^p$，令 $\mathbf{y} = \mathbf{g}(\mathbf{x})$，$\mathbf{z} = \mathbf{f}(\mathbf{y}) = \mathbf{f}(\mathbf{g}(\mathbf{x}))$。若 $\mathbf{f}$ 在 $\mathbf{y}$ 处可微，$\mathbf{g}$ 在 $\mathbf{x}$ 处可微，则：

$$
\frac{\partial z_k}{\partial x_j} = \sum_{i=1}^{m} \frac{\partial z_k}{\partial y_i} \cdot \frac{\partial y_i}{\partial x_j}
$$

或矩阵形式：$\mathbf{J}_{\mathbf{f} \circ \mathbf{g}} = \mathbf{J}_f \cdot \mathbf{J}_g$

**证明**：

**Step 1：定义复合函数**

令 $\mathbf{h} = \mathbf{f} \circ \mathbf{g}$，即 $\mathbf{h}(\mathbf{x}) = \mathbf{f}(\mathbf{g}(\mathbf{x}))$。

**Step 2：利用可微性的定义**

由于 $\mathbf{g}$ 在 $\mathbf{x}$ 处可微：

$$
\mathbf{g}(\mathbf{x} + \Delta\mathbf{x}) = \mathbf{g}(\mathbf{x}) + \mathbf{J}_g(\mathbf{x}) \Delta\mathbf{x} + \mathbf{o}(\|\Delta\mathbf{x}\|)
$$

由于 $\mathbf{f}$ 在 $\mathbf{y} = \mathbf{g}(\mathbf{x})$ 处可微：

$$
\mathbf{f}(\mathbf{y} + \Delta\mathbf{y}) = \mathbf{f}(\mathbf{y}) + \mathbf{J}_f(\mathbf{y}) \Delta\mathbf{y} + \mathbf{o}(\|\Delta\mathbf{y}\|)
$$

**Step 3：复合**

令 $\Delta\mathbf{y} = \mathbf{g}(\mathbf{x} + \Delta\mathbf{x}) - \mathbf{g}(\mathbf{x}) = \mathbf{J}_g \Delta\mathbf{x} + \mathbf{o}(\|\Delta\mathbf{x}\|)$

$$
\mathbf{h}(\mathbf{x} + \Delta\mathbf{x}) = \mathbf{f}(\mathbf{y} + \Delta\mathbf{y}) = \mathbf{f}(\mathbf{y}) + \mathbf{J}_f \Delta\mathbf{y} + \mathbf{o}(\|\Delta\mathbf{y}\|)
$$

**Step 4：代入并整理**

$$
\mathbf{h}(\mathbf{x} + \Delta\mathbf{x}) = \mathbf{f}(\mathbf{y}) + \mathbf{J}_f (\mathbf{J}_g \Delta\mathbf{x} + \mathbf{o}(\|\Delta\mathbf{x}\|)) + \mathbf{o}(\|\Delta\mathbf{y}\|)
$$

$$
= \mathbf{h}(\mathbf{x}) + \mathbf{J}_f \mathbf{J}_g \Delta\mathbf{x} + \mathbf{o}(\|\Delta\mathbf{x}\|)
$$

**Step 5：得出结论**

由可微性的定义，$\mathbf{h}$ 在 $\mathbf{x}$ 处可微，且雅可比矩阵为：

$$
\boxed{\mathbf{J}_h = \mathbf{J}_f \cdot \mathbf{J}_g}
$$

分量形式即：

$$
\boxed{\frac{\partial z_k}{\partial x_j} = \sum_{i=1}^{m} \frac{\partial z_k}{\partial y_i} \cdot \frac{\partial y_i}{\partial x_j}}
$$
...
**反向传播就是"追责"**：
1. 从最终损失开始
2. 向后逐层追溯：每层应该承担多少"责任"（梯度）
3. 每个人（参数）根据自己的"责任"进行调整
...
反向传播（计算梯度）：
步骤1：∂L/∂y = 2(y-t) = 2(7-5) = 4

步骤2：∂L/∂w = ∂L/∂y × ∂y/∂w = 4 × x = 4 × 2 = 8

步骤3：∂L/∂b = ∂L/∂y × ∂y/∂b = 4 × 1 = 4

参数更新（学习率η=0.1）：
w_new = w - η × ∂L/∂w = 3 - 0.1×8 = 2.2
b_new = b - η × ∂L/∂b = 1 - 0.1×4 = 0.6

新的预测：y_new = 2.2×2 + 0.6 = 5
新的损失：L_new = (5-5)² = 0  ← 完美！
```
