...
| 策略 | 数列类型 | 特点 |
|------|---------|------|
| **线性减速** | 等差数列 | 每步减少固定量，平稳但可能太慢 |
| **指数减速** | 等比数列 | 开始快后面慢，最常用 |
| **余弦减速** | 三角数列 | 先快后慢再平缓，Transformer最爱 |
...
信息逐渐"衰减"，就像数列 1, 0.8, 0.64, 0.51, 0.41...
这就是 RNN 的"记忆"特性——越久远的信息记得越少！
```

### 位置编码就像"给每个座位编号"

Transformer 的位置编码就像**电影院座位号**：
...
每个位置都有唯一的"编码"，让模型知道哪个词在前、哪个在后。
只不过 Transformer 用的是正弦/余弦函数来编码！
```
...
**场景**：初始学习率 $\eta_0 = 0.1$，衰减率 $\gamma = 0.9$，每1000步衰减一次

| 步数 t | 衰减次数 | 学习率计算 | 学习率值 |
|--------|---------|-----------|---------|
| 0-999 | 0 | $0.1 \times 0.9^0$ | 0.1000 |
| 1000-1999 | 1 | $0.1 \times 0.9^1$ | 0.0900 |
| 2000-2999 | 2 | $0.1 \times 0.9^2$ | 0.0810 |
| 3000-3999 | 3 | $0.1 \times 0.9^3$ | 0.0729 |
| 5000-5999 | 5 | $0.1 \times 0.9^5$ | 0.0590 |
| 10000-10999 | 10 | $0.1 \times 0.9^{10}$ | 0.0349 |

**观察**：
- 前1000步：学习率是0.1（快速学习）
- 到第10000步：学习率降到约0.035（精细调整）
- 最终会趋近于0（收敛）

这就像**半衰期**：每过一段时间，学习率就变成原来的90%！
