...
想象你是一个房产中介，想根据房屋面积预测房价。你收集了一些数据：
...
**观察规律**：每增加 1 平米，价格大约增加 3 万元。

这就是**线性回归**的核心思想：找一条直线，让所有数据点都尽量靠近这条线。
...
怎么找到最佳直线？**让所有点的误差（到直线的距离）的平方和最小**。

就像你要在教室里选一个座位，让到所有同学的距离总和最小一样。

### 偏差-方差权衡 = 死记硬背 vs 理解原理

| 学习方式 | 类比 | 机器学习 |
|---------|------|---------|
| **死记硬背** | 考试原题全对，新题不会做 | **高方差（过拟合）**：训练集完美，测试集很差 |
| **只看大纲** | 考试和练习都不太准确 | **高偏差（欠拟合）**：训练集和测试集都差 |
| **理解原理** | 既能做原题也能做新题 | **平衡**：泛化能力强 |
...
│     "抛了10次，全是正面"                                     │
│                                                            │
│  3. 后验（Posterior）：根据证据更新后的看法                   │
│     "这硬币可能有问题的，正面概率可能接近90%"                  │
└────────────────────────────────────────────────────────────┘
...
**目标**：找到直线 $y = \beta_0 + \beta_1 x$（截距 + 斜率）

**第1步：计算均值**
$$
\bar{x} = \frac{1+2+3+4}{4} = 2.5 \text{ 小时}
$$
$$
\bar{y} = \frac{50+60+70+80}{4} = 65 \text{ 分}
$$
...
|-------|-------|----------------|----------------|----------------------------------|---------------------|
| 1 | 50 | -1.5 | -15 | 22.5 | 2.25 |
| 2 | 60 | -0.5 | -5 | 2.5 | 0.25 |
| 3 | 70 | 0.5 | 5 | 2.5 | 0.25 |
| 4 | 80 | 1.5 | 15 | 22.5 | 2.25 |
| **合计** | | | | **50** | **5** |

$$
\beta_1 = \frac{50}{5} = 10 \text{ （每多学1小时，成绩提高10分）}
$$

**第3步：计算截距 $\beta_0$**
$$
\beta_0 = \bar{y} - \beta_1 \bar{x} = 65 - 10 \times 2.5 = 40
$$

**最终方程**：$y = 40 + 10x$

**预测**：如果学习5小时，预计成绩 = 40 + 10×5 = 90分
