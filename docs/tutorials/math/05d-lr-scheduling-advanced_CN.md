# 第五章（d）：学习率调度与高级技巧

学习率是深度学习训练中最重要的超参数。合适的学习率调度策略可以显著提高训练效率和最终性能。本节将介绍常用的学习率调度策略、二阶优化方法和一些高级训练技巧。

---

## 目录

1. [学习率的重要性](#学习率的重要性)
2. [常用学习率调度策略](#常用学习率调度策略)
3. [二阶优化方法](#二阶优化方法)
4. [约束优化](#约束优化)
5. [高级训练技巧](#高级训练技巧)
6. [在深度学习中的应用](#在深度学习中的应用)

---

## 学习率的重要性

### 🎯 生活类比：开车的油门

学习率就像汽车的**油门**：
- **油门踩太猛**（学习率太大）：车子失控，到处乱撞
- **油门踩太轻**（学习率太小）：车子蠕行，永远到不了终点
- **油门适中**：稳稳地开到目的地

**更具体的类比**：
- **初期**：在陌生路段，需要开慢点（小学习率）→ 不对！应该是先开快点找方向
- **中期**：熟悉路况了，正常速度（中等学习率）
- **后期**：快到了，减速停车（小学习率，精细调整）

### 📖 通俗理解

学习率 = **每一步迈多大的步子**

| 学习率 | 类比 | 结果 |
|--------|------|------|
| 太大 | 迈大步乱跳 | 在目标周围震荡，无法收敛 |
| 太小 | 蚂蚁爬 | 收敛太慢，可能陷入局部最优 |
| 刚好 | 正常走路 | 稳步到达目标 |

### 学习率对训练的影响

```
学习率太大：          学习率太小：          学习率合适：

  ↗ ↖ ↗ ↖              ·                    ↘
 ↗   ↖   ↗            · ·                   ↘
↗ 发散  ↖            · · ·  慢              ↘ 稳定收敛
                     ·     ·                  ↘
                    ·       ·                  ↘
```

### 学习率与训练阶段

| 阶段 | 学习率 | 目标 |
|------|--------|------|
| 初期 | 较大 | 快速下降 |
| 中期 | 适中 | 稳定优化 |
| 后期 | 较小 | 精细调优 |

---

## 常用学习率调度策略

### 1. 阶梯式衰减 (Step Decay)

### 🎯 生活类比：打折促销

想象商场打折：
- **第1-2周**：全场8折
- **第3-4周**：全场5折
- **第5周后**：全场3折

**学习率阶梯衰减也一样**：每隔一段时间，学习率"打折"一次！

```
时间线：
|--0.1--|--0.1--|--0.01--|--0.01--|--0.001--|
        ↓               ↓                ↓
      打折            再打折           最后打折
```

**公式**：

$$
\eta_t = \eta_0 \cdot \gamma^{\lfloor t / T \rfloor}
$$

每 $T$ 个 epoch 将学习率乘以 $\gamma$。

**示例**：初始 $\eta_0 = 0.1$，每 30 个 epoch 乘以 0.1

```
Epoch 0-29:   lr = 0.1
Epoch 30-59:  lr = 0.01
Epoch 60-89:  lr = 0.001
```

```python
class StepLR:
    """阶梯式学习率衰减"""
    
    def __init__(self, optimizer, step_size, gamma=0.1):
        self.optimizer = optimizer
        self.step_size = step_size
        self.gamma = gamma
        self.epoch = 0
    
    def step(self):
        """更新学习率"""
        self.epoch += 1
        if self.epoch % self.step_size == 0:
            self.optimizer.lr *= self.gamma
    
    def get_lr(self):
        return self.optimizer.lr
```

### 2. 指数衰减 (Exponential Decay)

**公式**：

$$
\eta_t = \eta_0 \cdot \gamma^t
$$

每个 epoch 学习率乘以 $\gamma$。

```python
class ExponentialLR:
    """指数学习率衰减"""
    
    def __init__(self, optimizer, gamma=0.95):
        self.optimizer = optimizer
        self.gamma = gamma
        self.epoch = 0
        self.base_lr = optimizer.lr
    
    def step(self):
        self.epoch += 1
        self.optimizer.lr = self.base_lr * (self.gamma ** self.epoch)
    
    def get_lr(self):
        return self.optimizer.lr
```

### 3. 余弦退火 (Cosine Annealing)

**公式**：

$$
\eta_t = \eta_{\min} + \frac{1}{2}(\eta_0 - \eta_{\min})\left(1 + \cos\left(\frac{t}{T}\pi\right)\right)
$$

**特点**：
- 初期学习率下降较慢
- 中期下降最快
- 后期趋于平稳

```python
import numpy as np

class CosineAnnealingLR:
    """余弦退火学习率调度"""
    
    def __init__(self, optimizer, T_max, eta_min=0):
        self.optimizer = optimizer
        self.T_max = T_max
        self.eta_min = eta_min
        self.eta_max = optimizer.lr
        self.epoch = 0
    
    def step(self):
        self.epoch += 1
        self.optimizer.lr = self.eta_min + 0.5 * (self.eta_max - self.eta_min) * \
                           (1 + np.cos(np.pi * self.epoch / self.T_max))
    
    def get_lr(self):
        return self.optimizer.lr

# 可视化
def plot_cosine_annealing():
    import matplotlib.pyplot as plt
    
    epochs = 100
    t = np.arange(1, epochs + 1)
    
    eta_max, eta_min = 0.1, 1e-6
    cosine_lr = eta_min + 0.5 * (eta_max - eta_min) * (1 + np.cos(np.pi * t / epochs))
    
    plt.figure(figsize=(10, 6))
    plt.plot(t, cosine_lr, linewidth=2)
    plt.xlabel('Epoch')
    plt.ylabel('Learning Rate')
    plt.title('Cosine Annealing Learning Rate')
    plt.grid(True)
    plt.show()

plot_cosine_annealing()
```

### 4. 线性预热 (Linear Warmup)

**动机**：训练初期参数随机，大学习率可能导致不稳定。

**策略**：前 $T_{\text{warmup}}$ 步线性增加学习率：

$$
\eta_t = \eta_{\max} \cdot \frac{t}{T_{\text{warmup}}}, \quad t \leq T_{\text{warmup}}
$$

```python
class LinearWarmup:
    """线性预热"""
    
    def __init__(self, optimizer, warmup_steps, target_lr):
        self.optimizer = optimizer
        self.warmup_steps = warmup_steps
        self.target_lr = target_lr
        self.step_count = 0
    
    def step(self):
        self.step_count += 1
        if self.step_count <= self.warmup_steps:
            self.optimizer.lr = self.target_lr * self.step_count / self.warmup_steps
    
    def is_warmup_done(self):
        return self.step_count >= self.warmup_steps
```

### 5. Warmup + 余弦退火

Transformer 训练的标准配置：

```python
class CosineWarmupScheduler:
    """带预热的余弦退火调度器"""
    
    def __init__(self, optimizer, warmup_epochs, max_epochs, eta_min=1e-6):
        self.optimizer = optimizer
        self.warmup_epochs = warmup_epochs
        self.max_epochs = max_epochs
        self.eta_min = eta_min
        self.eta_max = optimizer.lr
        self.epoch = 0
    
    def step(self):
        self.epoch += 1
        if self.epoch <= self.warmup_epochs:
            # 线性预热
            self.optimizer.lr = self.eta_max * self.epoch / self.warmup_epochs
        else:
            # 余弦退火
            progress = (self.epoch - self.warmup_epochs) / (self.max_epochs - self.warmup_epochs)
            self.optimizer.lr = self.eta_min + 0.5 * (self.eta_max - self.eta_min) * \
                               (1 + np.cos(np.pi * progress))
    
    def get_lr(self):
        return self.optimizer.lr

# 可视化
def plot_warmup_cosine():
    import matplotlib.pyplot as plt
    
    epochs = 100
    warmup = 10
    eta_max, eta_min = 0.1, 1e-6
    
    lr = []
    for t in range(1, epochs + 1):
        if t <= warmup:
            lr.append(eta_max * t / warmup)
        else:
            progress = (t - warmup) / (epochs - warmup)
            lr.append(eta_min + 0.5 * (eta_max - eta_min) * (1 + np.cos(np.pi * progress)))
    
    plt.figure(figsize=(10, 6))
    plt.plot(range(1, epochs + 1), lr, linewidth=2)
    plt.axvline(x=warmup, color='r', linestyle='--', label=f'Warmup end (epoch {warmup})')
    plt.xlabel('Epoch')
    plt.ylabel('Learning Rate')
    plt.title('Warmup + Cosine Annealing')
    plt.legend()
    plt.grid(True)
    plt.show()

plot_warmup_cosine()
```

### 6. Reduce on Plateau

当验证损失停止下降时，降低学习率：

```python
class ReduceLROnPlateau:
    """验证损失停滞时降低学习率"""
    
    def __init__(self, optimizer, mode='min', factor=0.1, patience=10, min_lr=1e-6):
        self.optimizer = optimizer
        self.mode = mode
        self.factor = factor
        self.patience = patience
        self.min_lr = min_lr
        self.best = float('inf') if mode == 'min' else float('-inf')
        self.bad_epochs = 0
    
    def step(self, metric):
        """根据指标更新学习率"""
        if self.mode == 'min':
            improved = metric < self.best
        else:
            improved = metric > self.best
        
        if improved:
            self.best = metric
            self.bad_epochs = 0
        else:
            self.bad_epochs += 1
            if self.bad_epochs >= self.patience:
                self.optimizer.lr = max(self.optimizer.lr * self.factor, self.min_lr)
                self.bad_epochs = 0
                print(f"Learning rate reduced to {self.optimizer.lr}")
```

### 7. 循环学习率 (Cyclical Learning Rate)

学习率在范围内周期性变化：

$$
\eta_t = \eta_{\min} + \frac{1}{2}(\eta_{\max} - \eta_{\min})(1 + \cos(\frac{2\pi t}{T}))
$$

```python
class CyclicLR:
    """循环学习率"""
    
    def __init__(self, optimizer, base_lr, max_lr, step_size_up, step_size_down=None):
        self.optimizer = optimizer
        self.base_lr = base_lr
        self.max_lr = max_lr
        self.step_size_up = step_size_up
        self.step_size_down = step_size_down or step_size_up
        self.step_count = 0
    
    def step(self):
        self.step_count += 1
        cycle = self.step_size_up + self.step_size_down
        
        if self.step_count % cycle < self.step_size_up:
            # 上升阶段
            pct = (self.step_count % cycle) / self.step_size_up
            self.optimizer.lr = self.base_lr + pct * (self.max_lr - self.base_lr)
        else:
            # 下降阶段
            pct = ((self.step_count % cycle) - self.step_size_up) / self.step_size_down
            self.optimizer.lr = self.max_lr - pct * (self.max_lr - self.base_lr)
```

### 学习率调度可视化

```python
def plot_all_schedulers():
    """可视化所有学习率调度策略"""
    import matplotlib.pyplot as plt
    
    epochs = 100
    warmup = 10
    t = np.arange(1, epochs + 1)
    eta_max, eta_min = 0.1, 1e-6
    
    # 1. 阶梯衰减
    step_lr = 0.1 * (0.1 ** (t // 30))
    
    # 2. 指数衰减
    exp_lr = 0.1 * (0.97 ** t)
    
    # 3. 余弦退火
    cosine_lr = eta_min + 0.5 * (eta_max - eta_min) * (1 + np.cos(np.pi * t / epochs))
    
    # 4. Warmup + 余弦
    warmup_cosine = np.zeros(epochs)
    for i in range(epochs):
        if i < warmup:
            warmup_cosine[i] = eta_max * (i + 1) / warmup
        else:
            progress = (i - warmup) / (epochs - warmup)
            warmup_cosine[i] = eta_min + 0.5 * (eta_max - eta_min) * (1 + np.cos(np.pi * progress))
    
    # 5. 循环学习率
    cycle = 20
    cyclic = []
    for i in range(epochs):
        pct = (i % cycle) / cycle
        if pct < 0.5:
            cyclic.append(eta_min + 2 * pct * (eta_max - eta_min))
        else:
            cyclic.append(eta_max - 2 * (pct - 0.5) * (eta_max - eta_min))
    
    fig, axes = plt.subplots(2, 3, figsize=(15, 8))
    
    axes[0, 0].plot(t, step_lr, linewidth=2)
    axes[0, 0].set_title('Step Decay')
    
    axes[0, 1].plot(t, exp_lr, linewidth=2)
    axes[0, 1].set_title('Exponential Decay')
    
    axes[0, 2].plot(t, cosine_lr, linewidth=2)
    axes[0, 2].set_title('Cosine Annealing')
    
    axes[1, 0].plot(t, warmup_cosine, linewidth=2)
    axes[1, 0].axvline(x=warmup, color='r', linestyle='--', alpha=0.5)
    axes[1, 0].set_title('Warmup + Cosine')
    
    axes[1, 1].plot(t, cyclic, linewidth=2)
    axes[1, 1].set_title('Cyclic LR')
    
    axes[1, 2].plot(t, cosine_lr, label='Cosine', linewidth=2)
    axes[1, 2].plot(t, warmup_cosine, label='Warmup+Cosine', linewidth=2)
    axes[1, 2].legend()
    axes[1, 2].set_title('Comparison')
    
    for ax in axes.flat:
        ax.set_xlabel('Epoch')
        ax.set_ylabel('Learning Rate')
        ax.grid(True)
    
    plt.tight_layout()
    plt.show()

plot_all_schedulers()
```

---

## 二阶优化方法

### 牛顿法

**原理**：利用二阶导数（Hessian）信息：

$$
\mathbf{x}_{t+1} = \mathbf{x}_t - [\nabla^2 f(\mathbf{x}_t)]^{-1} \nabla f(\mathbf{x}_t)
$$

**推导**：二阶泰勒展开：

$$
f(\mathbf{x}) \approx f(\mathbf{x}_t) + \nabla f(\mathbf{x}_t)^\top (\mathbf{x} - \mathbf{x}_t) + \frac{1}{2}(\mathbf{x} - \mathbf{x}_t)^\top \nabla^2 f(\mathbf{x}_t) (\mathbf{x} - \mathbf{x}_t)
$$

对 $\mathbf{x}$ 求导并令其为零，即得牛顿法更新公式。

**优点**：二次收敛（非常快）

**缺点**：
- 计算和存储 Hessian：$O(n^2)$
- 求逆：$O(n^3)$
- 需要正定 Hessian

### 牛顿法二次收敛性的证明

**定理**：设 $f$ 二阶连续可微，$\mathbf{x}^*$ 是 $f$ 的局部极小点，Hessian 矩阵 $\nabla^2 f(\mathbf{x}^*)$ 正定且满足 Lipschitz 条件。若初始点 $\mathbf{x}_0$ 充分接近 $\mathbf{x}^*$，则牛顿法具有**二次收敛**：

$$
\|\mathbf{x}_{t+1} - \mathbf{x}^*\| \leq C \|\mathbf{x}_t - \mathbf{x}^*\|^2
$$

**证明**：

**Step 1：定义误差向量**

令 $\mathbf{e}_t = \mathbf{x}_t - \mathbf{x}^*$ 为第 $t$ 次迭代的误差。

**Step 2：利用最优性条件**

在最优解处，$\nabla f(\mathbf{x}^*) = \mathbf{0}$。

对 $\nabla f(\mathbf{x}_t)$ 在 $\mathbf{x}^*$ 处泰勒展开：

$$
\nabla f(\mathbf{x}_t) = \nabla f(\mathbf{x}^*) + \nabla^2 f(\mathbf{x}^*)(\mathbf{x}_t - \mathbf{x}^*) + O(\|\mathbf{x}_t - \mathbf{x}^*\|^2)
$$

由于 $\nabla f(\mathbf{x}^*) = \mathbf{0}$，有：

$$
\nabla f(\mathbf{x}_t) = \nabla^2 f(\mathbf{x}^*) \mathbf{e}_t + O(\|\mathbf{e}_t\|^2)
$$

**Step 3：分析牛顿法更新**

牛顿法更新：$\mathbf{x}_{t+1} = \mathbf{x}_t - [\nabla^2 f(\mathbf{x}_t)]^{-1} \nabla f(\mathbf{x}_t)$

误差递推：

$$
\mathbf{e}_{t+1} = \mathbf{x}_{t+1} - \mathbf{x}^* = \mathbf{e}_t - [\nabla^2 f(\mathbf{x}_t)]^{-1} \nabla f(\mathbf{x}_t)
$$

**Step 4：代入梯度表达式**

代入 Step 2 的结果：

$$
\mathbf{e}_{t+1} = \mathbf{e}_t - [\nabla^2 f(\mathbf{x}_t)]^{-1} [\nabla^2 f(\mathbf{x}^*) \mathbf{e}_t + O(\|\mathbf{e}_t\|^2)]
$$

**Step 5：利用 Hessian 连续性**

由于 $\nabla^2 f$ 连续且 $\mathbf{x}_t$ 接近 $\mathbf{x}^*$，有 $\nabla^2 f(\mathbf{x}_t) \approx \nabla^2 f(\mathbf{x}^*)$。

令 $\mathbf{H}_t = \nabla^2 f(\mathbf{x}_t)$，$\mathbf{H}^* = \nabla^2 f(\mathbf{x}^*)$，则：

$$
[\mathbf{H}_t]^{-1} \mathbf{H}^* = \mathbf{I} + O(\|\mathbf{e}_t\|)
$$

因此：

$$
\mathbf{e}_{t+1} = \mathbf{e}_t - (\mathbf{I} + O(\|\mathbf{e}_t\|)) \mathbf{e}_t + O(\|\mathbf{e}_t\|^2) = O(\|\mathbf{e}_t\|^2)
$$

**Step 6：得出收敛速率**

存在常数 $C > 0$，使得：

$$
\boxed{\|\mathbf{e}_{t+1}\| \leq C \|\mathbf{e}_t\|^2}
$$

这就是**二次收敛**的定义：每步迭代将误差的阶数提高一次。

**直观理解**：
- 如果 $\|\mathbf{e}_t\| = 0.1$，则 $\|\mathbf{e}_{t+1}\| \approx C \times 0.01 = 0.01C$
- 如果 $\|\mathbf{e}_{t+1}\| = 0.01$，则 $\|\mathbf{e}_{t+2}\| \approx C \times 0.0001 = 0.0001C$
- 正确数字的位数大约每步翻倍！

```python
def newton_method(f, grad_f, hess_f, x0, max_iters=100, tol=1e-6):
    """牛顿法"""
    x = x0.copy()
    history = [x.copy()]
    
    for i in range(max_iters):
        g = grad_f(x)
        
        if np.linalg.norm(g) < tol:
            print(f"在 {i} 次迭代后收敛")
            break
        
        H = hess_f(x)
        
        # 求解 H * d = -g
        try:
            d = np.linalg.solve(H, -g)
        except np.linalg.LinAlgError:
            # Hessian 奇异，使用伪逆
            d = -np.linalg.pinv(H) @ g
        
        x = x + d
        history.append(x.copy())
    
    return x, history

# 示例：Rosenbrock 函数
def rosenbrock(x):
    return 100 * (x[1] - x[0]**2)**2 + (1 - x[0])**2

def rosenbrock_grad(x):
    return np.array([
        -400 * x[0] * (x[1] - x[0]**2) - 2 * (1 - x[0]),
        200 * (x[1] - x[0]**2)
    ])

def rosenbrock_hess(x):
    return np.array([
        [1200 * x[0]**2 - 400 * x[1] + 2, -400 * x[0]],
        [-400 * x[0], 200]
    ])

x0 = np.array([-1.0, 1.0])
x_opt, history = newton_method(rosenbrock, rosenbrock_grad, rosenbrock_hess, x0)
print(f"最优解: {x_opt}")
print(f"迭代次数: {len(history)}")
```

### 拟牛顿法：BFGS

避免直接计算和求逆 Hessian，通过迭代更新逆 Hessian 的近似：

$$
\mathbf{B}_{t+1} = \mathbf{B}_t - \frac{\mathbf{B}_t \mathbf{s}_t \mathbf{s}_t^\top \mathbf{B}_t}{\mathbf{s}_t^\top \mathbf{B}_t \mathbf{s}_t} + \frac{\mathbf{y}_t \mathbf{y}_t^\top}{\mathbf{y}_t^\top \mathbf{s}_t}
$$

其中 $\mathbf{s}_t = \mathbf{x}_{t+1} - \mathbf{x}_t$，$\mathbf{y}_t = \nabla f(\mathbf{x}_{t+1}) - \nabla f(\mathbf{x}_t)$。

### BFGS 更新公式的推导

**目标**：找到一个对称正定矩阵 $\mathbf{B}_{t+1}$ 作为 $\nabla^2 f(\mathbf{x}_{t+1})$ 的近似，满足**割线条件**。

**Step 1：割线条件 (Secant Condition)**

对 $\nabla f$ 在 $\mathbf{x}_t$ 处积分：

$$
\nabla f(\mathbf{x}_{t+1}) = \nabla f(\mathbf{x}_t) + \int_0^1 \nabla^2 f(\mathbf{x}_t + \tau \mathbf{s}_t) \mathbf{s}_t \, d\tau
$$

因此：

$$
\mathbf{y}_t = \nabla f(\mathbf{x}_{t+1}) - \nabla f(\mathbf{x}_t) \approx \nabla^2 f(\mathbf{x}_{t+1}) \mathbf{s}_t
$$

我们要求 $\mathbf{B}_{t+1}$ 满足：

$$
\boxed{\mathbf{B}_{t+1} \mathbf{s}_t = \mathbf{y}_t} \quad \text{(割线条件)}
$$

**Step 2：BFGS 更新形式**

设更新形式为秩二修正：

$$
\mathbf{B}_{t+1} = \mathbf{B}_t + \alpha \mathbf{u} \mathbf{u}^\top + \beta \mathbf{v} \mathbf{v}^\top
$$

其中 $\mathbf{u}$ 和 $\mathbf{v}$ 是待定向量，$\alpha$ 和 $\beta$ 是待定系数。

**Step 3：确定参数**

选择 $\mathbf{u} = \mathbf{y}_t$ 和 $\mathbf{v} = \mathbf{B}_t \mathbf{s}_t$，代入割线条件：

$$
\mathbf{B}_{t+1} \mathbf{s}_t = \mathbf{B}_t \mathbf{s}_t + \alpha \mathbf{y}_t (\mathbf{y}_t^\top \mathbf{s}_t) + \beta \mathbf{B}_t \mathbf{s}_t (\mathbf{s}_t^\top \mathbf{B}_t \mathbf{s}_t) = \mathbf{y}_t
$$

**Step 4：求解系数**

比较两边：
- 系数 $\alpha$：$\alpha (\mathbf{y}_t^\top \mathbf{s}_t) = 1$，故 $\alpha = \frac{1}{\mathbf{y}_t^\top \mathbf{s}_t}$
- 系数 $\beta$：$(1 + \beta \mathbf{s}_t^\top \mathbf{B}_t \mathbf{s}_t) = 0$，故 $\beta = -\frac{1}{\mathbf{s}_t^\top \mathbf{B}_t \mathbf{s}_t}$

**Step 5：得出 BFGS 更新公式**

将 $\alpha$ 和 $\beta$ 代入：

$$
\boxed{\mathbf{B}_{t+1} = \mathbf{B}_t - \frac{\mathbf{B}_t \mathbf{s}_t \mathbf{s}_t^\top \mathbf{B}_t}{\mathbf{s}_t^\top \mathbf{B}_t \mathbf{s}_t} + \frac{\mathbf{y}_t \mathbf{y}_t^\top}{\mathbf{y}_t^\top \mathbf{s}_t}}
$$

**BFGS 的性质**：

1. **保持正定性**：若 $\mathbf{y}_t^\top \mathbf{s}_t > 0$（曲率条件），$\mathbf{B}_{t+1}$ 正定
2. **超线性收敛**：在适当条件下，BFGS 收敛速率是超线性的
3. **自校正**：算法自动调整 Hessian 近似的尺度

**曲率条件的保证**：使用 Wolfe 条件进行线搜索可确保 $\mathbf{y}_t^\top \mathbf{s}_t > 0$。

### L-BFGS

Limited-memory BFGS：只存储最近 $m$ 个 $(\mathbf{s}_t, \mathbf{y}_t)$ 对，内存从 $O(n^2)$ 降到 $O(mn)$。

### 深度学习中的二阶方法

| 方法 | 复杂度 | 适用性 |
|------|--------|--------|
| 牛顿法 | $O(n^3)$ | 小规模问题 |
| BFGS | $O(n^2)$ | 中等规模 |
| L-BFGS | $O(mn)$ | 大规模 |

**深度学习中的挑战**：
- 参数数量巨大（百万甚至数十亿）
- Hessian 计算和存储不可行
- 通常使用一阶方法 + 巧妙的学习率调度

---

## 约束优化

### 投影梯度下降

对于约束集 $C$，在梯度下降后投影回可行域：

$$
\mathbf{x}_{t+1} = \text{Proj}_C(\mathbf{x}_t - \eta \nabla f(\mathbf{x}_t))
$$

**常用投影**：

1. **Box 约束** $[l, u]$：

$$
\text{Proj}_{[l,u]}(x) = \max(l, \min(u, x))
$$

2. **L2 球** $\{\mathbf{x} : \|\mathbf{x}\|_2 \leq r\}$：

$$
\text{Proj}(\mathbf{x}) = \frac{r \mathbf{x}}{\max(r, \|\mathbf{x}\|_2)}
$$

```python
def project_to_ball(x, radius=1.0):
    """投影到 L2 球"""
    norm = np.linalg.norm(x)
    if norm <= radius:
        return x
    return radius * x / norm

def project_to_box(x, lower, upper):
    """投影到 box 约束"""
    return np.clip(x, lower, upper)

def projected_gradient_descent(f, grad_f, x0, project_func, 
                                learning_rate=0.01, max_iters=1000):
    """投影梯度下降"""
    x = x0.copy()
    history = [x.copy()]
    
    for _ in range(max_iters):
        g = grad_f(x)
        x = project_func(x - learning_rate * g)
        history.append(x.copy())
    
    return x, history
```

### 权重裁剪

在深度学习中，常用于保持权重范数有界：

```python
def weight_clipping(model, max_norm=1.0):
    """权重裁剪"""
    for param in model.parameters():
        norm = np.linalg.norm(param)
        if norm > max_norm:
            param *= max_norm / norm
```

---

## 高级训练技巧

### 梯度裁剪

防止梯度爆炸：

```python
def clip_grad_norm_(grads, max_norm):
    """梯度范数裁剪"""
    total_norm = np.sqrt(sum(np.sum(g ** 2) for g in grads.values()))
    if total_norm > max_norm:
        scale = max_norm / total_norm
        for key in grads:
            grads[key] *= scale
    return total_norm

def clip_grad_value_(grads, clip_value):
    """梯度值裁剪"""
    for key in grads:
        grads[key] = np.clip(grads[key], -clip_value, clip_value)
```

### 学习率查找器 (LR Finder)

从小学习率开始，逐步增大，记录损失变化：

```python
def lr_finder(model, optimizer, train_loader, min_lr=1e-6, max_lr=1, num_steps=100):
    """学习率查找器"""
    lrs = np.logspace(np.log10(min_lr), np.log10(max_lr), num_steps)
    losses = []
    
    for i, lr in enumerate(lrs):
        optimizer.lr = lr
        
        # 单步训练
        for batch_x, batch_y in train_loader:
            output = model(batch_x)
            loss = model.compute_loss(output, batch_y)
            grads = model.backward()
            optimizer.step(model.params, grads)
            losses.append(loss)
            break
        
        if i > 0 and losses[-1] > 2 * losses[0]:
            break
    
    return lrs[:len(losses)], losses
```

### 混合精度训练

使用 FP16 加速训练，同时保持 FP32 精度：

```python
class MixedPrecisionTrainer:
    """混合精度训练（简化示例）"""
    
    def __init__(self, model, optimizer):
        self.model = model
        self.optimizer = optimizer
        # FP32 主权重副本
        self.master_params = {k: v.astype(np.float32) for k, v in model.params.items()}
    
    def step(self, grads):
        # 在 FP32 中累积梯度
        for key in self.master_params:
            self.master_params[key] -= self.optimizer.lr * grads[key].astype(np.float32)
        
        # 同步到 FP16
        for key in self.model.params:
            self.model.params[key] = self.master_params[key].astype(np.float16)
```

---

## 在深度学习中的应用

### nanotorch 中的学习率调度器

```python
from nanotorch.optim import SGD, AdamW
from nanotorch.optim import StepLR, CosineAnnealingLR, CosineWarmupScheduler, ReduceLROnPlateau

# 创建模型和优化器
model = create_model()
optimizer = AdamW(model.parameters(), lr=1e-3, weight_decay=0.01)

# 选择学习率调度器

# 选项 1: 阶梯衰减
scheduler = StepLR(optimizer, step_size=30, gamma=0.1)

# 选项 2: 余弦退火
scheduler = CosineAnnealingLR(optimizer, T_max=100, eta_min=1e-6)

# 选项 3: Warmup + 余弦（推荐用于 Transformer）
scheduler = CosineWarmupScheduler(optimizer, warmup_epochs=5, max_epochs=100)

# 选项 4: Reduce on Plateau
scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.1, patience=5)

# 训练循环
for epoch in range(100):
    train_loss = train_one_epoch(model, optimizer, train_loader)
    val_loss = validate(model, val_loader)
    
    # 更新学习率
    if isinstance(scheduler, ReduceLROnPlateau):
        scheduler.step(val_loss)
    else:
        scheduler.step()
    
    print(f"Epoch {epoch}, LR: {scheduler.get_lr():.6f}, Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}")
```

### 完整训练示例

```python
def train_with_scheduler():
    """带学习率调度的完整训练示例"""
    from nanotorch.optim import AdamW, CosineWarmupScheduler
    
    # 模型、优化器、调度器
    model = create_model()
    optimizer = AdamW(model.parameters(), lr=1e-3, weight_decay=0.01)
    scheduler = CosineWarmupScheduler(
        optimizer, 
        warmup_epochs=5, 
        max_epochs=100,
        eta_min=1e-6
    )
    
    best_val_loss = float('inf')
    
    for epoch in range(100):
        # 训练
        model.train()
        train_loss = 0
        for batch_x, batch_y in train_loader:
            output = model(Tensor(batch_x))
            loss = criterion(output, Tensor(batch_y))
            
            optimizer.zero_grad()
            loss.backward()
            
            # 梯度裁剪
            clip_grad_norm_(model.grads(), max_norm=1.0)
            
            optimizer.step()
            train_loss += loss.item()
        
        # 验证
        model.eval()
        val_loss = validate(model, val_loader)
        
        # 更新学习率
        scheduler.step()
        
        # 保存最佳模型
        if val_loss < best_val_loss:
            best_val_loss = val_loss
            save_model(model, 'best_model.npz')
        
        print(f"Epoch {epoch+1}/100, LR: {scheduler.get_lr():.6f}, "
              f"Train: {train_loss/len(train_loader):.4f}, "
              f"Val: {val_loss:.4f}")
```

---

## 小结

本节介绍了学习率调度和高级技巧：

### 学习率调度策略

| 策略 | 公式/方法 | 适用场景 |
|------|----------|----------|
| Step Decay | $\eta \cdot \gamma^{\lfloor t/T \rfloor}$ | 通用 |
| Cosine Annealing | $\eta_{\min} + \frac{1}{2}(\eta_0-\eta_{\min})(1+\cos(\pi t/T))$ | 通用 |
| Warmup + Cosine | 先线性增加再余弦退火 | Transformer |
| Reduce on Plateau | 验证损失停滞时降低 | 需监控的场景 |

### 二阶方法

| 方法 | 收敛率 | 复杂度 |
|------|--------|--------|
| 牛顿法 | 二次 | $O(n^3)$ |
| BFGS | 超线性 | $O(n^2)$ |
| L-BFGS | 超线性 | $O(mn)$ |

### 实践建议

1. **Transformer 训练**：AdamW + Warmup + Cosine
2. **CNN 训练**：SGD + Momentum + Step/Cosine
3. **监控学习率**：记录每次更新的学习率
4. **梯度裁剪**：RNN、Transformer 训练必备
5. **学习率查找器**：不确定时用于确定初始学习率

---

**上一节**：[自适应学习率方法](05c-自适应学习率方法.md)

**下一章**：[第六章：初等函数](06-elementary-functions.md) - 学习指数、对数、激活函数等深度学习中的数学函数。

**返回**：[第五章：最优化方法](05-optimization.md) | [数学基础教程目录](../math-fundamentals.md)
