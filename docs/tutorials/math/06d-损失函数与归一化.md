...
**MSE 计算**：
```
误差平方和 = 100 + 400 + 100 = 600
MSE = 600 / 3 = 200
```

**解释**：MSE=200，意味着平均每个预测偏差约 $\sqrt{200} \approx 14$ 万元

### 📖 为什么用平方而不是绝对值？

1. **数学方便**：平方函数处处可微
2. **惩罚大误差**：误差10变成100，误差20变成400——大误差被放大惩罚！
3. **概率解释**：假设误差服从正态分布，MSE就是最大似然估计
...
**场景**：你的老板给两种扣工资方案：
- **MSE方案**：误差1分钟扣1元，误差10分钟扣100元（平方惩罚）
- **MAE方案**：误差1分钟扣1元，误差10分钟扣10元（线性惩罚）

**你会选哪个？**
- 如果你经常犯小错 → MSE可能更好
- 如果你偶尔犯大错 → MAE更好（不会被一次大错"开除"）
