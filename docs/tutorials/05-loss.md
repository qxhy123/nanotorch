...
**损失函数，就是那个"扣分标准"**。它告诉神经网络：你的预测离正确答案有多远，差距在哪里，该如何改进。

```
预测值：[0.9, 0.05, 0.05]  ← 模型说"90%是猫"
真实值：[1.0, 0.00, 0.00]  ← 答案确实是猫

损失函数：0.1 × log(0.9) ≈ 0.01
         "答对了，但不够自信，扣 0.01 分"
```

每一次训练，模型都在"参加考试"。损失函数给它打分，梯度告诉它如何改进。一遍又一遍，直到...满分。

**损失函数，是模型的严师，也是它的指路明灯。**
