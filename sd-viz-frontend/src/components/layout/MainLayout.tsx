...
import { Brain, Waves, Box, Eye, Type, Zap, Sliders, Sparkles, Home, Moon, Sun } from 'lucide-react';

const navigation = [
  { name: 'Architecture', path: '/', icon: Home },
  { name: 'Diffusion', path: '/diffusion', icon: Waves },
  { name: 'UNet', path: '/unet', icon: Box },
  { name: 'Attention', path: '/attention', icon: Eye },
  { name: 'Text Encoder', path: '/text-encoder', icon: Type },
  { name: 'Sampling', path: '/sampling', icon: Zap },
  { name: 'Latent Space', path: '/latent-space', icon: Box },
  { name: 'Control', path: '/control', icon: Sliders },
    ...
    <div className="min-h-screen bg-background text-foreground">
        ...
        <div className="container mx-auto max-w-screen-2xl px-4">
          <div className="flex h-16 items-center justify-between gap-4">
            ...
            <Link to="/" className="flex items-center space-x-2 shrink-0">
              ...
              <span className="font-bold text-lg">SD-Viz</span>
            </Link>
      ...
      <footer className="mt-12 border-t py-6">
        <div className="container mx-auto px-4 text-center text-sm text-muted-foreground">
          <p>SD-Viz - Stable Diffusion Visualization Platform</p>
          <p className="text-xs mt-1">Built with React + TypeScript + Three.js</p>
        </div>
      </footer>
    </div>
  );
