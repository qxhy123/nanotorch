...
from nanotorch.detection.yolo_v9 import YOLOv9, YOLOv9Loss, build_yolov9
...
def demo_architecture():
    print("=" * 60)
    print("YOLO v9 Architecture Demo (PGI)")
    print("=" * 60)
    ...
    print("\nKey Features:")
    print("   - GELAN (Generalized Efficient Layer Aggregation Network)")
    print("   - PGI (Programmable Gradient Information)")
    print("   - Re-parameterization")

def train(args):
    print("=" * 60)
    print("YOLO v9 Training Demo")
    print("=" * 60)
    model = build_yolov9(num_classes=args.num_classes, input_size=args.image_size)
    num_params = sum(p.data.size for p in model.parameters())
    ...
    np.savez('yolov9_best.npz', **model.state_dict())
    print(f"Model saved to: yolov9_best.npz")
    return model

def inference(args):
    print("=" * 60)
    print("YOLO v9 Inference Demo")
    print("=" * 60)
    model = build_yolov9(num_classes=args.num_classes, input_size=args.image_size)
    if os.path.exists('yolov9_best.npz'):
        model.load_state_dict(dict(np.load('yolov9_best.npz')))
...
def main():
    parser = argparse.ArgumentParser(description='YOLO v9 Demo')
