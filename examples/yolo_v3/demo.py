#!/usr/bin/env python3
"""
YOLO v3 Demo - Training and Inference Example

This demo showcases how to use nanotorch to train and evaluate YOLO v3
for object detection on synthetic data.

Usage:
    python examples/yolo_v3/demo.py --mode train --epochs 3
...
def demo_architecture():
    print("=" * 60)
    print("YOLO v3 Architecture Demo")
    print("=" * 60)
    ...
    print("   - Large scale (52x52): Small objects")
    print("   - Medium scale (26x26): Medium objects")
    print("   - Small scale (13x13): Large objects")


def train(args):
    print("=" * 60)
    print("YOLO v3 Training Demo")
    print("=" * 60)
    ...
    if use_tiny:
        model = YOLOv3Tiny(num_classes=args.num_classes, input_size=image_size)
        print(f"Using YOLOv3Tiny with input size {image_size}")
    else:
        print("WARNING: Full YOLOv3 requires significant memory")
        model = YOLOv3(num_classes=args.num_classes, input_size=image_size)
        print(f"Using YOLOv3 with input size {image_size}")
    ...
    np.savez('yolov3_best.npz', **state_dict)
    print(f"Model saved to: yolov3_best.npz")
...
def inference(args):
    ...
    if os.path.exists('yolov3_best.npz'):
        print("Loading trained weights from yolov3_best.npz...")
        state_dict = dict(np.load('yolov3_best.npz'))
...
def main():
    ...
    parser.add_argument('--full', action='store_true', help='Use full YOLOv3 (requires more memory)')
