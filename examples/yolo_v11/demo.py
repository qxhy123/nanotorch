...
from nanotorch.detection.yolo_v11 import YOLOv11, YOLOv11Loss, build_yolov11
...
def demo_architecture():
    print("=" * 60)
    print("YOLO v11 Architecture Demo (Ultralytics Latest)")
    print("=" * 60)
    model = build_yolov11(num_classes=80, input_size=224)
    ...
    print("\nKey Features:")
    print("   - C3k2 modules (faster C3k)")
    print("   - Enhanced architecture")
    print("   - State-of-the-art accuracy-speed tradeoff")

def train(args):
    print("=" * 60)
    print("YOLO v11 Training Demo")
    print("=" * 60)
    model = build_yolov11(num_classes=args.num_classes, input_size=args.image_size)
    num_params = sum(p.data.size for p in model.parameters())
    ...
    np.savez('yolov11_best.npz', **model.state_dict())
    print(f"Model saved to: yolov11_best.npz")
    return model

def inference(args):
    print("=" * 60)
    print("YOLO v11 Inference Demo")
    print("=" * 60)
    model = build_yolov11(num_classes=args.num_classes, input_size=args.image_size)
    if os.path.exists('yolov11_best.npz'):
        model.load_state_dict(dict(np.load('yolov11_best.npz')))
...
def main():
    parser = argparse.ArgumentParser(description='YOLO v11 Demo')
