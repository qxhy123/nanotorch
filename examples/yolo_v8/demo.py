...
from nanotorch.detection.yolo_v8 import YOLOv8, YOLOv8Loss, build_yolov8
...
def demo_architecture():
    print("=" * 60)
    print("YOLO v8 Architecture Demo (Ultralytics)")
    print("=" * 60)
    ...
    print("\nKey Features:")
    print("   - C2f modules (faster CSP)")
    print("   - Anchor-free detection")
    print("   - Decoupled head")

def train(args):
    print("=" * 60)
    print("YOLO v8 Training Demo")
    print("=" * 60)
    model = build_yolov8(num_classes=args.num_classes, input_size=args.image_size)
    num_params = sum(p.data.size for p in model.parameters())
    ...
    np.savez('yolov8_best.npz', **model.state_dict())
    print(f"Model saved to: yolov8_best.npz")
    return model

def inference(args):
    print("=" * 60)
    print("YOLO v8 Inference Demo")
    print("=" * 60)
    model = build_yolov8(num_classes=args.num_classes, input_size=args.image_size)
    if os.path.exists('yolov8_best.npz'):
        model.load_state_dict(dict(np.load('yolov8_best.npz')))
...
def main():
    parser = argparse.ArgumentParser(description='YOLO v8 Demo')
